...
GameplayStateGuest::GameplayStateGuest(GameDataRef data, int tcpSocketClient)
    : _data(data), _tcpSocketClient(tcpSocketClient), _running(true) {
  ...
  _udpManager = new UdpNetworkManager(
      -1, _tcpSocketClient); // tcpSocketServer is -1 for client
  if (!_udpManager->Initialize()) {
    ...
    // or a crash if it returns false and _udpManager is used later without
    // being valid.
  }
...
void GameplayStateGuest::InitPlayer(Player &p, float x, float y) {
  p.render.bodySprite->setOrigin(
      p.render.bodySprite->getLocalBounds().width / 2,
  ...
  p.render.gunSprite->setOrigin(
      5.f, p.render.gunSprite->getLocalBounds().height / 2);
...
void GameplayStateGuest::Init() {
  ...
  _player.render.bodySprite->setTexture(
      _data->assetManager.GetTexture("player"));
...
void GameplayStateGuest::HandleInput() {
  ...
  while (_data->window.pollEvent(event)) {
    ...
    if (event.type == sf::Event::MouseButtonPressed &&
      ...
      if (_udpManager) {
        ...
        std::cout << "Guest Sending Fire Message: " << msg
                  << std::endl; // LOG RAW MESSAGE
  ...
  if (_udpManager &&
      (_player.core.velocity.x != 0 || _player.core.velocity.y != 0)) {
    std::string posMsg =
        "pos;" + std::to_string(_player.core.hitbox.getPosition().x) + ";" +
...
void GameplayStateGuest::Update() {
  ...
  for (auto it = _bullets.begin(); it != _bullets.end();) {
    ...
    if (_enemy.core.hitbox.getGlobalBounds().intersects(
            it->hitbox.getGlobalBounds())) {
      // Informacja o trafieniu może być wysłana do hosta lub host sam to
      // wykryje
      it = _bullets.erase(it);
    } else if (it->sprite.getPosition().x < 0 ||
               it->sprite.getPosition().x > _windowSize.x ||
               it->sprite.getPosition().y < 0 ||
               it->sprite.getPosition().y > _windowSize.y) {
      it = _bullets.erase(it);
    } else {
      ++it;
    }
  ...
  for (auto it = _enemyBullets.begin(); it != _enemyBullets.end();) {
    ...
    if (_player.core.hitbox.getGlobalBounds().intersects(
            it->hitbox.getGlobalBounds())) {
      if (_udpManager)
        _udpManager->Send("hit_player"); // Informuj hosta o trafieniu
    ...
    if (_udpManager && _udpManager->HasMessages()) {
      ...
      if (typeEnd != std::string::npos) {
        ...
        std::lock_guard<std::mutex> lock(
            _dataMutex); // Lock before accessing shared data
        if (type == "pos_enemy") {
          ...
          std::cout << "Guest Received bullet (Thread). Parts size: "
                    << parts.size() << std::endl;
          if (parts.size() == 6) {
            try {
              Bullet bullet;
              bullet.sprite.setTexture(
                  _data->assetManager.GetTexture("bulletRed"));
              bullet.sprite.setScale(0.8f, 0.8f);
              bullet.sprite.setOrigin(bullet.sprite.getLocalBounds().width / 2,
                                      bullet.sprite.getLocalBounds().height /
                                          2);
              bullet.sprite.setPosition(std::stof(parts[0]),
                                        std::stof(parts[1]));
              bullet.velocity = {std::stof(parts[2]), std::stof(parts[3])};
              bullet.sprite.setRotation(std::stof(parts[4]));
              int owner = std::stoi(parts[5]);

              bullet.hitbox.setSize(
                  sf::Vector2f(bullet.sprite.getGlobalBounds().width * 0.8f,
                               bullet.sprite.getGlobalBounds().height *
                                   0.8f)); // Smaller hitbox
          ...
          // std::cout << "Guest Received round_over (Thread). Winner ID: " <<
          // data << std::endl;
          //  Handle round over logic, potentially setting a flag to be checked
          //  in Update()
...
void GameplayStateGuest::UpdateGunTransform(sf::Sprite *targetSprite,
                                            sf::Sprite *gunSprite) {
...
void GameplayStateGuest::UpdateGunRotation(sf::Sprite *targetSprite,
                                           sf::Sprite *gunSprite) {
...
void GameplayStateGuest::DisplayPlayerData(Player &p) {
  ...
  p.render.healthBarFill.setPosition(
      p.render.healthBarBackground.getPosition().x + 2.f,
...
void GameplayStateGuest::FireBullet(sf::Sprite *sourceSprite,
                                    sf::Sprite *gunSprite) {
  Bullet newBullet;
...
void GameplayStateGuest::ClearObjects() { _bullets.clear(); }
