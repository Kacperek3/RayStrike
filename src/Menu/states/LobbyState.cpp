...
LobbyState::LobbyState(const LobbyConfig config)
    : _config(config), _data(config.data) {
  ...
  _networkLobbyManager = new NetworkLobbyManager(_config.serverSocketForClient,
                                                 _config.clientSocket);
}
...
void LobbyState::Init() {
  ...
  _yourColor =
      _config.isHost ? sf::Color(255, 223, 0) : sf::Color(230, 230, 230);
  _enemyColor =
      _config.isHost ? sf::Color(230, 230, 230) : sf::Color(255, 223, 0);
  ...
  _minusNumOfRoundsIcon->setTexture(
      _data->assetManager.GetTexture("minusIcon"));
  _minusNumOfRoundsIcon->setPosition(180, 570);

  _data->assetManager.LoadTexture("plusIconHover",
                                  "assets/flair_plus_hover.png");
  ...
  _data->assetManager.LoadTexture("sendIconHover",
                                  "assets/flair_arrows_right_hover.png");
      ...
      if (event.mouseButton.button == sf::Mouse::Left) {
        ...
        if (_backButton->getGlobalBounds().contains(mousePos)) {
          ...
          if (_isHostReady && _isClientReady) {
            ...
            int socketToPass = _config.isHost ? _config.serverSocketForClient : _config.clientSocket;

            // 2. Uwolnij socket i zniszcz starego managera
            if (_networkLobbyManager) {
                _networkLobbyManager->ReleaseSocket(); // Ważne! Zatrzymuje wątek, ale nie zamyka połączenia
            ...
            if (_config.isHost) {
                _data->stateManager.AddState(StateRef(new GameplayStateHost(_data, socketToPass)), false);
            } else {
                _data->stateManager.AddState(StateRef(new GameplayStateGuest(_data, socketToPass)), false);
            } 
            return;
          }
          ...
          if (_gameSettings.numberOfRounds < 7) {
            ...
            _numberOfRoundsText->setString(
                "Number of rounds: " +
                std::to_string(_gameSettings.numberOfRounds));
            _networkLobbyManager->Send(
                "__NUMBER_OF_ROUNDS__" +
                std::to_string(_gameSettings.numberOfRounds));
        ...
        } else if (_minusNumOfRoundsIcon->getGlobalBounds().contains(
                       mousePos) &&
                   _config.isHost) {
          ...
          if (_gameSettings.timeLimit < 120) {
            ...
            _timeLimitText->setString(
                "Time limit: " + std::to_string(_gameSettings.timeLimit) +
                " sec");
...
void LobbyState::AddMessageToChat(const std::string &message,
...
void LobbyState::UpdateChatPositions() {
  ...
  for (auto it = _chatMessages.rbegin(); it != _chatMessages.rend(); ++it) {
    ...
    int numLines =
        std::count(msg->getString().begin(), msg->getString().end(), '\n') + 1;
    msg->setPosition(baseX + msgData.offsetX,
...
void LobbyState::Update() {
  if (!_networkLobbyManager) return;
  _tesseract->update();
    ...
    if (msg.find("__DISCONNECT") != std::string::npos) {
      ...
      if (pos != std::string::npos) {
        ...
        try {
          ...
          std::cerr << "Error parsing number of rounds: " << e.what()
                    << std::endl;
        }
      }
      ...
      if (pos != std::string::npos) {
        ...
        try {
          ...
          _timeLimitText->setString(
              "Time limit: " + std::to_string(newTimeLimit) + " sec");
        } catch (const std::exception &e) {
      ...
      if (pos != std::string::npos) {
        std::string receivedMessage = msg.substr(pos + prefix.length());
        AddMessageToChat(receivedMessage, sf::Color(255, 255, 255, 120), 14,
                         40);
      }
    }
...
void LobbyState::standartAnimation() {
  ...
  if (_data->inputManager.IsSpriteHoverAccurate(*_plusNumOfRoundsIcon,
                                                _data->window, 45)) {
    _plusNumOfRoundsIcon->setTexture(
        _data->assetManager.GetTexture("plusIconHover"));
  } else {
    _plusNumOfRoundsIcon->setTexture(
        _data->assetManager.GetTexture("plusIcon"));
  }
  if (_data->inputManager.IsSpriteHoverAccurate(*_plusTimeLimitIcon,
                                                _data->window, 45)) {
    _plusTimeLimitIcon->setTexture(
        _data->assetManager.GetTexture("plusIconHover"));
  } else {
    _plusTimeLimitIcon->setTexture(_data->assetManager.GetTexture("plusIcon"));
  }

  if (_data->inputManager.IsSpriteHoverAccurate(*_sendMessageIcon,
                                                _data->window, 20)) {
    _sendMessageIcon->setTexture(
        _data->assetManager.GetTexture("sendIconHover"));
  } else {
    _sendMessageIcon->setTexture(_data->assetManager.GetTexture("sendIcon"));
  }
