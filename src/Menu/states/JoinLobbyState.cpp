...
void JoinLobbyState::Init() {
  ...
  _data->assetManager.LoadTexture("reloadLobbiesButton",
                                  "assets/flair_arrow_3.png");
  _data->assetManager.LoadTexture("reloadLobbiesButtonHover",
                                  "assets/flair_arrow_3_hover.png");
  ...
  _reloadLobbiesButton->setTexture(
      _data->assetManager.GetTexture("reloadLobbiesButton"));
  ...
  while (_data->window.pollEvent(event)) {
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
      if (event.mouseButton.button == sf::Mouse::Left) {
        ...
        if (_data->inputManager.IsSpriteClicked(
                *_reloadLobbiesButton, sf::Mouse::Left, _data->window)) {
          ...
          std::sort(temp.begin(), temp.end(),
                    [](const LobbyInfo &a, const LobbyInfo &b) {
          ...
          for (auto background : _backgroundForOption)
            delete background;
          ...
          if (_joinButtons[i]->getGlobalBounds().contains(mousePos)) {
            if (i < _currentLobbies.size()) {
              ...
              if (_networkManager.connectToServer(lobby.ip, lobby.port)) {
                ...
                //_config.clientName = lobby.playerName; //! trzeba dodac w
                //jakis ciekawy sposob nazwe klienta
                _lobbyConfig.isHost = false;
                _data->stateManager.AddState(
                    StateRef(new LobbyState(_lobbyConfig)),
                    true); // !! pokminic jak
                //! usunac state aby przenioslo nas z powrotem do join lobby
                //! jednak by LobbyDiscovery sie nie zatrzymalo poki co przenosi
                //! nas do menu glownego
              } else {
...
void JoinLobbyState::standartAnimation() {
  ...
  if (_data->inputManager.IsSpriteHover(*_reloadLobbiesButton, _data->window)) {
    _reloadLobbiesButton->setTexture(
        _data->assetManager.GetTexture("reloadLobbiesButtonHover"));
  } else {
    _reloadLobbiesButton->setTexture(
        _data->assetManager.GetTexture("reloadLobbiesButton"));
  }
